...
*Last refreshed: 2026-04-27T18:00Z (nightly hunt #1) | Next review: 2026-05-03*
...
### Finding: 2026-04-27 — Tailwind CSS
- **Source:** https://github.com/tailwindlabs/tailwindcss-container-queries + https://tailwindcss.com/blog/tailwindcss-v3-2
- **Finding:** The official `@tailwindcss/container-queries` plugin (Tailwind Labs) is available for v3.2+ TODAY — provides `@container` parent marker plus `@xs:`/`@sm:`/`@md:`/`@lg:`/`@xl:`/`@2xl:`–`@7xl:` size variants and named container syntax (`@container/sidebar`, `@lg/sidebar:underline`). Memoria runs Tailwind v3.4, so container queries are actionable today via plugin install — no v4 migration prerequisite. Plugin is officially deprecated only in v4 (where `@container` is built-in).
- **Relevance:** Contradicts the framing in the existing v4 migration block (line 567-573) which lists native `@container` queries as a "v4 migration" task — implying we wait. The "card looks great in grid, breaks in 300px sidebar" failure mode that container queries fix is ALREADY a real risk for `EventCard`, `MagnetEventCard`, KPI tiles, and `PrintStation` queue cards on v3.4. Adopting the plugin now decouples component-vs-viewport responsiveness without a Tailwind major-version bump.
- **Action:** (1) Update the v4 migration block (line 571) to clarify: native `@container` is a v4 *simplification* (no plugin), but the capability is available today on v3.4 via `@tailwindcss/container-queries` — don't gate adoption on the v4 migration. (2) Decide policy: adopt plugin now OR explicitly defer to v4. Recommend NOT adopting yet — Memoria's current breakpoint pain is around iPhone-SE narrow widths (handled by `min-[Npx]:` arbitrary variants per Future Migrations entry, line 561) rather than parent-vs-viewport mismatch. Re-evaluate when `PrintStation` ships (kiosk-side cards rendered in variable sidebar widths). (3) **Same `container-type: size` gotcha applies on v3.4 plugin** (per existing v4 block) — never use `@container/{name}/size`, default `inline-size` only.
- **Status:** pending-review

### Finding: 2026-04-27 — Supabase Auth
- **Source:** https://github.com/supabase/supabase/issues/42527 (open, filed Feb 2026)
- **Finding:** `supabase.auth.resend({ type: 'signup', email })` always emits a confirmation email using the **implicit flow** (token-fragment URL) — even on a client configured with `flowType: 'pkce'`. The original `signUp()` correctly emits a PKCE `?code=` link, but a subsequent "resend" produces a `#access_token=...` fragment link instead. Symptom: the first confirmation email opens correctly (matches the `exchangeCodeForSession` callback handler); a resent email arrives at the same callback route but the handler sees no `code` param, silently fails, and the user sees a "link expired" UI even on a fresh click. Same inconsistency reported for `auth.resend({ type: 'email_change' })`.
- **Relevance:** Memoria's `signUp` flow uses PKCE (per existing memory line 417-420 — "PKCE single-use, 5-min TTL" pitfall). Any "send the confirmation email again" UX (host re-registration retry, "didn't get it" button, OAuth-link recovery) will deliver an implicit-flow link that the existing PKCE callback handler can't parse. NOT a security regression — the resent link is still a legitimate confirmation token — but a silent UX dead-end. Affects every Memoria host who needs a second confirmation email.
- **Action:** (1) Audit the codebase for any `supabase.auth.resend(...)` call site — `Grep -r "auth.resend" src/` to confirm presence; if currently absent (likely — confirmed-email-resend UX may not exist yet), add a PRD note that this gotcha must be handled when the feature ships. (2) When implementing resend UX, the callback handler MUST detect both `?code=` (PKCE, standard signUp/recovery) AND `#access_token=` URL-fragment (implicit, resend-only) — wire two parsing paths until upstream fixes #42527. (3) Hebrew error fallback: if BOTH parsers fail, render `הקישור פג תוקף — בקש/י קישור חדש` (matches the existing PKCE single-use fallback message — same UX surface). (4) Subscribe to issue #42527 for upstream fix; remove the implicit-flow parser when resolved.
- **Status:** pending-review
