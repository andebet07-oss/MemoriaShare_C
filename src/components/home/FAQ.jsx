    ...
    <section id="faq" className="py-24" dir="rtl">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-6 text-gray-100">שאלות נפוצות</h2>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">כל מה שרציתם לדעת על Memoria במקום אחד.</p>
        </div>
        
        <div className="max-w-3xl mx-auto">
          <Accordion type="single" collapsible className="w-full">
            {faqItems.map((item, index) => (
              <AccordionItem key={index} value={`item-${index}`} className="group bg-neutral-900/50 backdrop-blur-sm border border-white/10 rounded-2xl mb-4 transition-all duration-300 hover:border-gray-400/20 data-[state=open]:bg-neutral-800/60 data-[state=open]:border-gray-400/30">
                <AccordionTrigger className="w-full text-right text-lg font-semibold text-white p-6 hover:no-underline">
                  <div className="flex justify-between items-center w-full">
                    <span>{item.question}</span>
                    <div className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 group-data-[state=open]:bg-white/20 transition-all duration-300">
                        <Plus className="w-5 h-5 text-white transition-transform duration-300 group-data-[state=open]:rotate-45" />
                    </div>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="text-gray-300 text-base leading-relaxed px-6 pb-6">
                  {item.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
